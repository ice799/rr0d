...
/*
int mystrcpy(char *buf,char *texto);


int copystring(char *buf,char *texto)
{
	int i=0;
  while (*texto!=0) {*buf=*texto;buf++;texto++;i++;};
  return i;
};
*/
...
//	int i;
    ...
    /*updata nprefix de l'instr*/
    ins.nprefix = data - origdata;
    for (i=0; i<(*p)->operands; i++) {
